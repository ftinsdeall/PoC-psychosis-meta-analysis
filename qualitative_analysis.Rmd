...
```{r - polyI:C dose category administered}
polyICcat_effect <- SMD.g %>% 
  select(effect_id, polyIC.dose.cat) %>% 
  group_by(polyIC.dose.cat) %>% 
  tally()
```
29 studies administered 'high' doses of polyIC
22 studies administered 'low' doses of polyIC
193 studies administered 'moderate' doses of polyIC
3 studies did not report dose of polyIC
...
## Gestational stage polyIC
```{r}
gestational_stage <- SMD.g %>% 
...
```
3 effects from NA
75 effect sizes from early mid gestation 
137 effect sizes from mid-late gestation 
32 effect sizes from late gestation 
...
```{r - acute or chronic polyIC}
acute_or_chronic <- SMD.g %>% 
  select(effect_id, acute_or_chronic) %>% 
  group_by(acute_or_chronic) %>% 
  tally()
```
4 effect sizes came from a chronic polyIC administration protocol (5mg/kg ip od for 5 days starting GD12) 
243 effect sizes came from acute polyIC administration protocols 

1 study used chronic polyIC admnistration protocol 
47 studies used acute polyIC administration protocol 


## Studies reporting %PPI across multiple pre-pulse intensities vs. for single pre-pulse intensities 

```{r - averaged or specific prepulse PPI}
averaged_prepulse <- SMD.g %>% 
  select(study_id_str, is_this_ppi_for_a_specific_pre_pulse_intensity_or_averaged_across_multiple) %>% 
  distinct() %>% 
  group_by(is_this_ppi_for_a_specific_pre_pulse_intensity_or_averaged_across_multiple) %>% 
  tally()
```

29 effect sizes were from PPI values averaged across multiple pre-pulse intensities
218 effect sizes were from PPI values for specific pre-pulse intensities 

13 studies reported PPI values averaged across multiple pre-pulse intensities 
37 studies reported PPI values for specific pre-pulse intensities 

## Studies reporting %PPI across multiple pulse intensities vs. for single pulse intensities 

```{r - averaged or specific pulse PPI}
averaged_pulse <- SMD.g %>% 
  select(effect_id, is_this_ppi_for_a_specific_pulse_intensity_or_is_it_averaged_across_multiple) %>% 
  group_by(is_this_ppi_for_a_specific_pulse_intensity_or_is_it_averaged_across_multiple) %>% 
  tally()
```
1 effect size came from a PPI value averaged across multiple pulse intensities 
246 effect sizes came from PPI values for specific pulse intensities 

1 study reported PPI values averaged across multiple pulse intensities 
47 studies reported PPI values for specific pulse intensities 


## Range of backgound intensities used 

```{r - background intensities}
background_intensity <- SMD.g %>% 
  select(study_id_str, background_db) %>% 
  distinct() %>% 
  group_by(background_db) %>% 
  tally()
```
4 studies used 60dB
17 studies used 65dB
6 studies used 68dB
17 studies used 70dB 
2 studies used 71dB 
1 study used 74dB
1 study did not report the level of background noise 

## Pre-pulse durations 

```{r - prepulse durations}
prepulse_duration <- SMD.g %>% 
  select(study_id_str, prepulse_duration_ms) %>% 
  distinct() %>% 
  group_by(prepulse_duration_ms) %>% 
  tally()
```

170 effect sizes came from 20ms prepulse duration 
6 effect sizes came from 30ms prepulse duration 
19 effect sizes came from 40ms prepulse duration 
52 effect sizes came from %PPI measurement protocols that did not report prepulse duration 

33 studies used 20ms prepulse duration 
1 study used 30ms prepulse duration 
4 studies used 40ms prepulse duration 
10 studies did not report prepulse duration


## Pulse intensity above background used (will need to separate column to get pulses for averaged pulses)

```{r - create reduced dataset with split columns (pusle)}

all_pulses_w <- SMD.g %>% 
  select(study_id_str, pulse_intensity_d_b, background_db) %>% 
  distinct() %>% 
  separate_wider_delim(pulse_intensity_d_b, delim = ' ,', names = c('pulse1', 'pulse2', 'pulse3'), too_few = 'align_start')

all_pulses <- all_pulses %>% 
  pivot_longer(cols = c('pulse1', 'pulse2', 'pulse3'), names_to = 'pulse') %>% 
  drop_na() %>% 
  mutate(value = as.numeric(value)) %>% 
  mutate(pulse_above_background = value - background_db)

pulse_above_background <- all_pulses %>% 
  select(study_id_str, pulse_above_background) %>% 
  distinct() %>% 
  group_by(pulse_above_background) %>% 
  tally()
  
```

5 studies used a pulse 35dB above background noise
4 studies used a pulse 40dB above background noise 
1 study used a pulse 42dB above background noise 
10 studies used a pulse 45dB above background noise 
1 study used a pulse 46dB above background noise 
2 studies used a pulse 49dB above background noise 
11 studies used a pulse 50dB above background noise 
5 studies used a pulse 52 dB above background noise 
15 studies used a pulse 55 DB above background noise 
1 study used a pulse 60dB above background noise 


## Pre-pulse intensity above background used (same as above)

```{r - create reduced dataset with split columns (prepulse)}

all_prepulses_w <- SMD.g %>% 
  select(study_id_str, prepulse_intensity_d_b, background_db) %>% 
  distinct() %>% 
  separate_wider_delim(prepulse_intensity_d_b, delim = ',', names = c('prepulse1', 'prepulse2', 'prepulse3', 'prepulse4', 'prepulse5'), too_few = 'align_start')

all_prepulses <- all_prepulses_w %>% 
  pivot_longer(cols = c('prepulse1', 'prepulse2', 'prepulse3', 'prepulse4', 'prepulse5'), names_to = 'prepulse') %>% 
  drop_na() %>% 
  mutate(value = as.numeric(value)) %>% 
  mutate(prepulse_above_background = value - background_db)

prepulse_above_background <- all_prepulses %>% 
  select(study_id_str, prepulse_above_background) %>% 
  distinct() %>% 
  group_by(prepulse_above_background) %>% 
  tally()
  
```

1 study used a prepulse that was -1dB below background noise 
5 studies used a prepulse 2dB above background noise 
7 studies used a prepulse 3dB above background noise
24 studies used a prepulse 4dB above background noise 
4 studies used a prepulse 5dB above background noise 
11 studies used a prepulse 6dB above background noise 
2 studies used a prepulse 7dB above background noise 
22 studies used a prepulse 8dB above background noise
2 studies used a prepulse 9dB above background noise 
10 studies used a prepulse 10dB above background noise 
1 study used a prepulse 11dB above background noise 
25 studies used a prepulse 12dB above background noise 
1 study used a prepulse 14 dB above backgrounf noise 
6 studies used a prepulse 15dB above background noise 
18 studies used a prepulse 16dB above background noise 
1 study used a prepulse 17dB above background noise 
5 studies used a prepulse 18dB above background noise 
3 studies used a prepulse 20dB above background noise 
1 study used a prepulse 25dB above background noise 
1 study used a prepulse 49dB above background noise 

## Interstimulus intervals used (will need to split column)

```{r - create reduced dataset with split columns (ITI), include=FALSE}

interstimulus <- SMD.g %>% 
  select(study_id_str, interstimulus_interval_ms) %>% 
  distinct() %>% 
  separate_wider_delim(interstimulus_interval_ms, delim = ',', names = c('ITI1', 'ITI2', 'ITI3', 'ITI4'), too_few = 'align_start')

all_interstimulus <- interstimulus %>% 
  pivot_longer(cols = c('ITI1', 'ITI2', 'ITI3', 'ITI4'), names_to = 'ITI') %>% 
  drop_na() %>% 
  select(-ITI) %>% 
  group_by(value) %>% 
  tally()
...
4 studies did not report interstimulus duration 
2 studies used an 8ms ITI
2 studies used a 16ms ITI 
3 studies used a 30ms ITI
2 studies used a 32ms ITI
5 studies used a 50ms ITI
2 studies used a 64ms ITI
1 study used a 70ms ITI
7 studies used an 80ms ITI
35 studies used a 100ms ITI
2 studies used a 128ms ITI 
2 studies used a 140ms ITI
2 studies used a 256ms ITI 
1 study used a 15000ms ITI 
...
For study characteristics, I want the title, authors, species of animal, strain, sex of cohorts, number of experiments, total number of animals, gestational stages of polyI:C administration, and developmental stages at PPI testing 
...
# Alluvium plots
```{r - fig.1 - alluvium plot showing heterogeneity across methodology of polyI:C disease model protocol for 7 main variables}
freq <- dat %>% 
  select(species_of_animal, sex_of_animals, poly_I_C_daily_dose_mg_kg, GD_first_administration, vehicle, acute_or_chronic, strain) %>% 
  table() %>% 
  as.data.frame() %>% 
  filter(Freq > '0')
  
is_alluvia_form(as.data.frame(freq), axes = 1:7, silent = TRUE)

alluvial_plot <- ggplot(data = freq, aes(axis1 = species_of_animal, axis2 = strain, axis3 = sex_of_animals, axis4 = poly_I_C_daily_dose_mg_kg,  axis5 = vehicle, axis6 = GD_first_administration, axis7 = acute_or_chronic, y = Freq)) + 
    geom_alluvium(aes(fill = strain, colour = strain)) + 
    geom_flow() + 
    geom_stratum() + 
  ...
  scale_x_discrete(limits = c("Species", "Strain", "Sex", "Dose of PolyI:C", "Vehicle", "GD of administration", "Acute or Chronic"), position = "top")
...
```{r - fig.1 - alluvium plot showing heterogeneity across methodology of PPI testing protocol main variables}
freq.2 <- dat %>% 
  select(strain, sex_of_animals, time, background_db, prepulse_duration_ms, pre_pulse_level_above_back, pulse_intensity_d_b, interstimulus_interval_ms) %>% 
  table() %>% 
  as.data.frame() %>% 
  filter(Freq > '0')
  
is_alluvia_form(as.data.frame(freq.2), axes = 1:8, silent = TRUE)

alluvial_plot.2 <- ggplot(data = freq.2, aes(axis1 = strain, axis2 = sex_of_animals, axis3 = time, axis4 = background_db,  axis5 = pre_pulse_level_above_back, axis6 = prepulse_duration_ms, axis7 = pulse_intensity_d_b, axis8 = interstimulus_interval_ms, y = Freq)) + 
    geom_alluvium(aes(fill = strain, colour = strain)) + 
    geom_flow() + 
    geom_stratum() + 
    geom_text(stat = "stratum", aes(label = after_stat(stratum)), size= 3) +
  ...
  scale_x_discrete(limits = c('Strain', 'Sex', 'PND of testing', 'Background Noise (dB)', 'PP level above background (dB)', 'PP duration', 'Pulse intensity (dB)', 'ITI'), position = "top")
